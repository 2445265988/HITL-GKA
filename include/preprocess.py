...
def enhance_triples(kg1, kg2, ents1, ents2):
    ...
    #  ents1 中的实体到 ents2 中的实体的映射
    links1 = dict(zip(ents1, ents2))
    #  ents2 中的实体到 ents1 中的实体的映射
    links2 = dict(zip(ents2, ents1))
    for h1, r1, t1 in kg1.triples:

        h2 = links1.get(h1, None)
        t2 = links1.get(t1, None)
        # 若KG1头尾实体都在对齐实体集合中，则链接对齐实体集合中的两个尾实体，与R1重新组成一个三元组，此为KG1的新的扩展三元组，并且该新的三元组不在KG2中
    ...
    for h2, r2, t2 in kg2.triples:
        ...
        # 若若KG2头尾实体都在对齐实体集合中，则链接对齐实体集合中的两个尾实体，与R2重新组成一个三元组，此为KG2的新的扩展三元组，并且该新的三元组不在KG1中
